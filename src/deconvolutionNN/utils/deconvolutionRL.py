...
def plotter(images, labels, cmap="jet", log=False):
    ...
    for i in range(0, n):
        if log:
            ax[i].imshow(
                images[i], norm=colors.LogNorm(), cmap=cmap
            )  # clim=(1,1000),cmap=cmap)
        else:
...
def load_h5_scan_to_npy(file_path, scan, plot=True):
    ...
    for filename in os.listdir(file_path_new)[:-1]:
        ...
        if points:
            ...
            print(
                "what format are the h5 files in relation to scan (i.e. each h5 file corresponds to a single line or a single point)?"
            )
            break
    dps = np.asarray(dps)
    return dps
