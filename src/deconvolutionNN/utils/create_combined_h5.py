    ...
    try:
        if os.path.exists(h5_file):
            ...
            if result is not None:
                ...
                print(
                    "Warning: Could not read existing H5 file after retries. Starting from scratch."
                )
...
def save_to_h5(output_file, result):
    ...
    if not success:
        print(
            f"Warning: Could not save scan {result['scan_number']} to H5 file after retries."
        )
...
def process_scan(
    scan_number, base_path, model, device, center, dpsize, mask, indices_file=None
):
    """Process a single scan"""
    try:
        # Load mask
        print(f"Loading scan {scan_number}")
        dps = ptNN_U.load_h5_scan_to_npy(
            base_path, scan_number, plot=False, point_data=True
        )
        ...
        if indices_file:
            ...
            if selected_indices is None:
                ...
                print(
                    f"Using {len(selected_indices)} selected indices for scan {scan_number}"
                )
        ...
        for i in range(n_batches):
            ...
            for j, dp in enumerate(batch_dps):
                cropped_dp = dp[
                    center[0] - dpsize // 2 : center[0] + dpsize // 2,
            ...
            input_list_log = 10 ** (
                np.array([t.numpy()[0][0] for t in input_tensors]) * sfs + bkgs
            )
...
def save_verification_plot(results, output_dir="plots"):
    ...
    try:
        ...
        deconvolved_data = np.where(
            ~deconvolved_data.mask, deconvolved_data + epsilon, deconvolved_data
        )
        preprocessed_data = np.where(
            ~preprocessed_data.mask, preprocessed_data + epsilon, preprocessed_data
        )
        ...
        plt.savefig(
            os.path.join(output_dir, f'scan_{results["scan_number"]}_summary_TEST.png')
        )
        plt.close()
    except Exception as e:
        print(
            f"Warning: Could not save verification plot for scan {results['scan_number']}: {str(e)}"
        )
...
def main():
    ...
    df = pd.read_csv(
        "/net/micdata/data2/12IDC/2025_Feb/misc/ZCB_9_3D_/ZCB9_3D_alignment_shifts_28nm.txt",
    ...
    print(
        f"Found {len(processed_scans)} already processed scans. {len(remaining_scans)} scans remaining."
    )

    # Add indices file parameter
    indices_file = (
        "/home/beams/PTYCHOSAXS/NN/ptychosaxsNN/utils/ZCB_9_3D_sample_indices.h5"
    )
    ...
    for scan_number in tqdm(remaining_scans, desc="Processing scans"):
        ...
        if scan_number != remaining_scans[-1]:  # Don't delay after the last scan
            print(
                f"\nWaiting {delay_between_scans} seconds before processing next scan...",
                flush=True,
            )
