...
scans = np.load(basepath2 / "2023_Mar/results/ML_recon/scans_585_to_765/phis.npz")[
    "scan"
]
print(phis, scans)
...
object_tests_new = loadmat(
    basepath2
    / f"2024_Dec/results/JM01_3D_/fly{probe_scan}/roi0_Ndp512/MLc_L1_p10_gInf_Ndp256_mom0.5_pc200_model_scale_rotation_shear_asymmetry_noModelCon_bg0.1_vi_mm/MLs_L1_p10_g200_Ndp378_pc200_model_scale_asymmetry_rotation_shear_maxPosError200nm_noModelCon_bg0.1_vi_mm/Niter1000.mat"
)
...
size = 64
psf = probe_tests_new_FT[
    probe_tests_new_FT.shape[0] // 2 - size : probe_tests_new_FT.shape[0] // 2 + size,
    probe_tests_new_FT.shape[1] // 2 - size : probe_tests_new_FT.shape[1] // 2 + size,
]  # zhihua
...
for index, scan in enumerate(tqdm(scans[10:12])):
    if scan in scans:
        ...
        object_tests_new = loadmat(
            basepath2
            / "2024_Dec/results/JM01_3D_/fly085/roi0_Ndp512/MLc_L1_p10_gInf_Ndp256_mom0.5_pc200_model_scale_rotation_shear_asymmetry_noModelCon_bg0.1_vi_mm/MLs_L1_p10_g200_Ndp378_pc200_model_scale_asymmetry_rotation_shear_maxPosError200nm_noModelCon_bg0.1_vi_mm/Niter1000.mat"
        )
        ...
        size = 64
        psf = probe_tests_new_FT[
            probe_tests_new_FT.shape[0] // 2 - size : probe_tests_new_FT.shape[0] // 2
            + size,
            probe_tests_new_FT.shape[1] // 2 - size : probe_tests_new_FT.shape[1] // 2
            + size,
        ]  # zhihua
        # crop probe
        probe_gray = (psf * 255 / np.max(psf)).astype(np.uint8)
        ...
        for i in range(0, len(dps[count])):
            for j in range(0, len(dps[count][i])):
                result_mtx[j][i][count] = result_cpu[i][
                    j
                ]  # loaded with transpose so need to flip again i <-> jlen
