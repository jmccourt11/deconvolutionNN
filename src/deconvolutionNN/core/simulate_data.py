...
def create_lattice(lattice_type, num_points_x, num_points_y, num_points_z, spacing):
    ...
    if lattice_type == "SC":
        ...
        fcc_points = np.vstack(
            [
                np.column_stack(
                    [xv.ravel() + spacing / 2, yv.ravel() + spacing / 2, zv.ravel()]
                ),
                np.column_stack(
                    [xv.ravel() + spacing / 2, yv.ravel(), zv.ravel() + spacing / 2]
                ),
                np.column_stack(
                    [xv.ravel(), yv.ravel() + spacing / 2, zv.ravel() + spacing / 2]
                ),
            ]
        )
        ...
        raise ValueError(
            "Unsupported lattice type. Supported types are: 'SC', 'BCC', 'FCC'"
        )
...
def form_factor_sphere(radius, q):
    ...
    return (
        3
        * (np.sin(q_norm * radius) - q_norm * radius * np.cos(q_norm * radius))
        / (q_norm**3 * radius**3)
    )
...
def form_factor_cube(side_length, q):
    ...
    return (
        np.sinc(q[0] * half_side / np.pi)
        * np.sinc(q[1] * half_side / np.pi)
        * np.sinc(q[2] * half_side / np.pi)
    )
...
def simulate_diffraction(
    lattice_points, q_values, form_factor_func, form_factor_params
):
    """
...
def project_to_2d_plane(q_values, q_z):
    ...
    return np.column_stack(
        [q_values[:, 0], q_values[:, 1], np.full(q_values.shape[0], q_z)]
    )
            ...
            for phi in phis:
                for theta in thetas:
                    for psi in psis:
                        ...
                        # Rotate lattice
                        rotated_lattice_points = rotate_lattice(
                            lattice_points, phi, theta, psi
                        )
                        ...
                        intensities = simulate_diffraction(
                            rotated_lattice_points, q_values_3d, f, (radius,)
                        )

                        # Plot the 3D lattice and the 2D diffraction pattern side by side
                        plot_side_by_side(
                            rotated_lattice_points, q_values_2d, intensities, q_max
                        )
                        ...
                        if save:
                            np.savez(
                                "/mnt/micdata2/12IDC/ptychosaxs/data/diff_sim/{}/output_ideal_{:05d}.npz".format(
                                    dir_num, count
                                ),
                                idealDP=intensities_image,
                            )
                            print(f"file saved to output_ideal_{count:05d}.npz")
